...
def subsequence_visualization(ll=[[1,5,"AAAAE",9], [2,16,"AAEAAAEAAAEAAAE",2], [2,19,"AAAEAAAEAAAEAAAEAAE",10], [3,16,"BVG",7], [4,12,"BVG",5]], protein_sequence="AAAEAAAEAAAEAAAEAAAE", protein_accession="PXSD78", enzyme="GluC"):
    ...
    indices = [i + 1 for i in range(len(ll))]
    
    # check if full, semi or non-enzyme specific 
    colors = []
    for l in ll:
        start_, stop_, seq_, psms_ = l
        if start_ == 0 or start_ == 1: n_term = True 
        elif protein_sequence[start_-1] in aas: n_term = True
        else: n_term = False
        if stop_ == len(protein_sequence): c_term = True
        elif seq_[-1] in aas: c_term = True
        else: c_term = False
        if n_term and c_term: colors.append("black")
        elif n_term and not c_term: colors.append("orange")
        elif c_term and not n_term: colors.append("red")
        elif not n_term and not c_term: colors.append("blue")


    plt.barh(y=indices, width=[e-s for e,s in zip(end,start)], left=start, color = colors)
    plt.xlabel("AA Position", fontsize=14)
    plt.ylabel("Peptides", fontsize=14)
    plt.title(protein_accession, fontsize=14)
...
#     plt.barh(y=indices, width=end, left=start, color = colors)
...
def proteoform_distribution(start, end, protein_sequence, protein_accession, ylabel="#Peptides"):
    plt.figure(dpi=120)
...
def truncations_pi_chart(numbers=[1232,2342,651,241]): 
    labels = ["Internal", "C-term", "N-term", "Full length"]
    ...
    def func(pct, numbers):
        absolute = int(round(pct/100.*np.sum(numbers),0))
    ...
    patches, texts, autotexts = plt.pie(
        x=numbers, 
        labels=labels,
        autopct=lambda pct: func(pct, numbers),
...
def protease_specificity_pi_chart(d={"A":12, "B": 34, "C": 15}): 
    labels = list(d.keys())
