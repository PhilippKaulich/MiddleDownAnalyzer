...
from PyQt5.QtWidgets import QWidget, QFileDialog, QTextEdit, QVBoxLayout, QScrollArea, QCompleter
...
class Ui(QtWidgets.QMainWindow):
    ...
    def initialization(self):
        self.a = AnalyzeCleavageSites.AnalyzeCleavageSites(self.fasta_file)
        ...
        completer_acc = QCompleter(list(all_accessions), self)
        self.lineedit_accession.setCompleter(completer_acc)
    ...
    def plot_specificity(self):
        print("Protease Specificity", self.protease_specificity)
        protease_specificity_pi_chart(self.protease_specificity)
        
        
    def show_sequence_coverage(self):
        protein_groups_properties = self.a.analyze_sequence_coverage(self.accession_proteoforms)
        self.a.visualize_dataframe(protein_groups_properties)
    ...
    def export_icelogo_data(self):
        file_filter = Settings.export_file_filter
        initialFilter = Settings.export_initialFilter
        file_name = QFileDialog.getSaveFileName(caption = 'Save IceLogo data as',
            directory = '', filter = file_filter, initialFilter = initialFilter)[0]
        self.data_icelogo_df.to_csv(file_name)
    ...
    def analyze_accession(self):
        ...
        accession = self.lineedit_accession.text()
...
class ProteinWindow(QWidget):
    def __init__(self):
        super().__init__()
        self.setFixedSize(640, 480)
        self.setWindowTitle("Identified Proteingroups")
        layout = QVBoxLayout()
        self.textedit = QTextEdit()
        layout.addWidget(self.textedit)
        self.setLayout(layout)
